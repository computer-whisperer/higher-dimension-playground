...
impl RenderContext {
    ...
    pub fn new_with_pixel_storage_layers(
        ...
        let damascene_overlay = swapchain.as_ref().map(|swapchain| {
            let mut runner =
                damascene_vulkano::Runner::new(device.clone(), queue.clone(), swapchain.image_format());
            runner.set_theme(damascene_core::Theme::radix_slate_blue_dark());
            runner.set_surface_size(window_size.width.max(1), window_size.height.max(1));
            DamasceneOverlay { runner }
        });
