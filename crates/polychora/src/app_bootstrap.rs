...
pub(super) fn run_cpu_render(_scene_preset: ScenePreset, args: &Args) {
    ...
    let material_resolver = polychora::content_registry::MaterialResolver::from_registry(&content_registry);
    let img = cpu_render::cpu_render(&instances, &model_tets, &params, &content_registry, &material_resolver);
